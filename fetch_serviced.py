...
async def get_page_data(session, page):
    data = {
        ...
        'stock': 'in_stock',
        'category_id': '91',
        'canonical': 'https://ekb.zenmod.shop/serviced/',
    ...
    async with session.post(url=url, headers=headers, cookies=cookies, data=data) as response:
        ...
        items = Atomizers.parse_raw(data)
...
async def gather_data():
    async with aiohttp.ClientSession() as session:
        response = await session.get('https://ekb.zenmod.shop/serviced/?stock=in_stock', headers=headers, cookies=cookies)
...
def main():
    ...
    if not os.path.isfile(f'jsons/atomizers.json'):
        with open('jsons/atomizers.json', 'w', encoding='utf8') as file:
            json.dump(atomizers, file, indent=4, ensure_ascii=False)
    else:
        with open('jsons/atomizers.json', 'r', encoding='utf8') as file:
            old_atomizers = json.load(file)
        try:
